...
const Slider = () => {
  ...
  function getTimeRemaining(endTime) {
    // Thêm tham số endTime vào hàm getTimeRemaining
  ...
  return (
    <div className="List_Slide">
      <div className="List_Slide_logo">
        <p>
          The 2025 International Conference on Research in Engineering and
          Technology
        </p>
      </div>
      <Carousel autoplay>
        ...
        {countdown && (
          <ul className="countdown">
            <li className="countdown-item">
              <span className="countdown-number days">{countdown?.days}</span>
              <span className="countdown-text">days</span>
            </li>
            <li className="countdown-item">
              <span className="countdown-number hours">{countdown?.hours}</span>
        ...
        {ImportantDate && (
          <ul>
            <li className="modify_color_red">
              <span className="old_day">
                {ImportantDate[0]?.changedate !== "" &&
                  formatDate(ImportantDate[0]?.date)}
              </span>{" "}
              {ImportantDate[0]?.changedate === ""
                ? formatDate(ImportantDate[0]?.date)
                : formatDate(ImportantDate[0]?.changedate)}
              : {ImportantDate[0]?.titledate}
            </li>

            <li>
              <span className="old_day">
                {ImportantDate[1]?.changedate !== "" &&
                  formatDate(ImportantDate[1]?.date)}
              </span>{" "}
              {ImportantDate[1]?.changedate === ""
                ? formatDate(ImportantDate[1]?.date)
                : formatDate(ImportantDate[1]?.changedate)}
              : {ImportantDate[1]?.titledate}
            </li>

            <li className="modify_color_yellow">
              <span className="old_day">
                {ImportantDate[2]?.changedate !== "" &&
                  formatDate(ImportantDate[2]?.date)}
              </span>{" "}
              {ImportantDate[2]?.changedate === ""
                ? formatDate(ImportantDate[2]?.date)
                : formatDate(ImportantDate[2]?.changedate)}
              : {ImportantDate[2]?.titledate}
            </li>
          </ul>
        )}
      </div>
      <div className="List_Slide_overplay"></div>
    </div>
  );
};
